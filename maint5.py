...
for i in range(n_epoch):
    ...
    for input_ids, attention_mask, decoder_input_ids, decoder_attention_mask in gen:
        ...
        if index % 50 == 0:
            print(tokenizer.batch_decode(decoder_input_ids, skip_special_tokens=True)[0])
            print(tokenizer.batch_decode(torch.softmax(logits, dim=2), skip_special_tokens=True)[0])
