...
with torch.no_grad():
    ...
    for input_ids, attention_mask, decoder_input_ids, decoder_attention_mask in cluster_loader:
        probability = torch.nn.functional.softmax(model.encoder(input_ids=input_ids, attention_mask=attention_mask).logits).detach().cpu().numpy().tolist()
        clusters_prob += probability
        if ind % 1000 == 0:
            print(len(clusters_prob))
        ind += 1
n_clusters = 5
clusters = KMeans(n_clusters=n_clusters).fit(np.array(clusters_prob).reshape(-1, 1)).labels_
batch_size = 5
