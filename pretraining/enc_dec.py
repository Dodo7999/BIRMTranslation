...
#
# print(model.config)
# # model.config.d_ff = 512
# model.config.d_model = 64
# print(model.config)
# model = MT5ForConditionalGeneration(model.config)
...
for i, path in enumerate(paths):
    ...
    for record in records:
        if record.text != '' and len(cluster) < 200_000:
            for text in split_string(record.text, chunk_size):
                if text != '' and len(val_set) > (i+1) * 5:
                    cluster.append(i)
                    # train_set.append(mask_spans(text))
                    train_set.append([text, text])
                elif text != '':
                    cluster_val.append(i)
                    # val_set.append(mask_spans(text))
                    val_set.append([text, text])
...
for i in range(n_epoch):
    ...
    for envs in train_loader:
        ...
        if index % 100 == 0:
            print(f"Count = {index}")
            print(
                f"Epoch = {i}, loss = {loss}, losses = {loss_t}, penalty = {penalty}, batch_index = {index}, lr = {opt.param_groups[0]['lr']}")
            if index % 100 == 0:
                model.eval()
